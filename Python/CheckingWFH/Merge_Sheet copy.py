        ...
        if data[x]["JAM"] != y:
            if int(abs(data[x-1]["JAM"] - data[x]["JAM"])) >= 3 and int(abs(data[x-1]["JAM"] - data[x]["JAM"])) < 23:     #klo ada data jam yang kosong berdempetan
                if data[x-1]["STATUS"] == "UPDATED":
                    ...
                    totalActivity_ = sum(jmlA)/len(jmlA)
                    totalUsage_ = sum(jmlU)/len(jmlU)

                    activityDempet = Activity(data,x,y,totalActivity=totalActivity_)
                    usageDempet = Usage(data,x,y,totalUsage=totalUsage_)

                    sheet.insert_row([interpolasi.makeREG(),y,device.rumus_Dempet(),interpolasi.makeWFH(),interpolasi.makeKey(),activityDempet.rumus_Dempet(),usageDempet.rumus_Dempet(),"UPDATED"],x+2)
                else:
                    for i in range(0,len(data)):
                        if data[i]["REG"] == data[x]["REG"] and data[i]["JAM"] == data[x]["JAM"] and data[i]["WFH_DATE"] == data[x]["WFH_DATE"]:
                            break
                    ...
                    activityDempet2 = Activity(data,x,y,totalActivity=totalActivity_)
                    usageDempet2 = Usage(data,x,y,totalUsage=totalUsage_)

                    sheet.insert_row([interpolasi.makeREG(),y,device.rumus_Dempet(),interpolasi.makeWFH(),interpolasi.makeKey(),activityDempet2.rumus_Dempet(),usageDempet2.rumus_Dempet(),"UPDATED"],x+2)
